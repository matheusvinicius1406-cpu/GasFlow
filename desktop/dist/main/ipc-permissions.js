...
function createPermissionGate(options = {}) {
    ...
    async function fetchPermissionsFromBackend() {
        ...
        if (!token)
            return [];
        try {
            const { status, body } = await fetchJson(`${backendBaseUrl}/auth/me`, { Authorization: `Bearer ${token}` });
            if (status !== 200 || !Array.isArray(body.permissions))
                return [];
...
//# sourceMappingURL=ipc-permissions.js.map

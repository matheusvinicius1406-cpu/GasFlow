...
exports.defaultExecFile = void 0;
exports.ensureScriptFile = ensureScriptFile;
exports.buildPrintArgs = buildPrintArgs;
exports.printRawBytes = printRawBytes;
...
function ensureScriptFile(dir = node_os_1.default.tmpdir()) {
    if (cachedScriptPath && node_fs_1.default.existsSync(cachedScriptPath))
        return cachedScriptPath;
    const target = node_path_1.default.join(dir, "gasflow-raw-print.ps1");
    node_fs_1.default.writeFileSync(target, RAW_PRINT_SCRIPT, "utf-8");
    cachedScriptPath = target;
    return target;
}
/** Args do PowerShell para uma impressão (exposto para teste). */
function buildPrintArgs(scriptPath, printerName, filePath) {
    return [
        "-NoProfile",
        "-NonInteractive",
        "-ExecutionPolicy",
 ...
 */
async function printRawBytes(printerName, data, opts = {}) {
    ...
    try {
        ...
        tmpFile = node_path_1.default.join(opts.tmpDir ?? node_os_1.default.tmpdir(), `gasflow-print-${process.pid}-${Date.now()}.bin`);
        node_fs_1.default.writeFileSync(tmpFile, data);
        const exec = opts.exec ?? exports.defaultExecFile;
...
//# sourceMappingURL=print-transport.js.map

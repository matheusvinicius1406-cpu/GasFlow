...
class AiSetupRunner {
    ...
    async runInner() {
        ...
        if (this.status.state === "unavailable")
            return this.status; // instalado mas não respondeu
    ...
    async promptInstallConsent() {
        if (!this.notifyOwner)
            return false; // sem diálogo → nunca instala
        try {
    ...
    async pullOnce(model) {
        ...
        return new Promise((resolve) => {
            ...
            const req = this.httpMod.request(`${this.baseUrl}/api/pull`, {
                ...
                res.on("data", (chunk) => {
                    for (const line of chunk.split("\n")) {
                        ...
                        try {
                            ...
                            if (total > 0)
                                lastPercent = Math.min(99, Math.round((done * 100) / total));
                            if (evt.error) {
                                finish(false);
                                return;
                            }
                            if (this.onProgress)
                                this.onProgress({ percent: lastPercent, bytes: done, total });
                        }
                ...
                res.on("end", () => {
                    if (this.onProgress)
                        this.onProgress({ percent: 100, bytes: 0, total: 0 });
                    finish(true);
                });
            });

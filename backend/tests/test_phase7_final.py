...
def seed_client(db, codigo="000001"):
    c = ClientModel(codigo=codigo, nome="Test", telefone="11999999999", rua="Rua A", numero="1", bairro="Centro")
    db.add(c)
    db.commit()
    return c
...
def test_multi_item_success(db):
    """Both sufficient → order confirmed, stock NOT debited (Decisão B3a:
    débito acontece na entrega DELIVERED, não no CONFIRMED)."""
    seed_product(db, "P00001", "Product A")
    db.add(ProductModel(codigo="P00002", nome="Product B", tipo="GAS", preco=50.0, estoque=0))
    db.commit()
    seed_client(db)
    ...
    # Decisão B3a: CONFIRMED não debita mais — estoque inalterado.
    inv_a = db.query(InventoryModel).filter(InventoryModel.product_codigo == "P00001").first()
    inv_b = db.query(InventoryModel).filter(InventoryModel.product_codigo == "P00002").first()
    assert inv_a.quantity == 10
    ...
    sales = (
        ...
        .filter(StockMovementModel.reference_id == order.codigo, StockMovementModel.type == "SALE")
        .count()
    )
    assert sales == 0
...
def test_cancel_pending_no_movement(db):
    ...
    order = CreateOrderUseCase(
        ...
        product_repo=SQLAlchemyProductRepository(db),
        inventory_repo=repo,
    ).execute({"client_codigo": "000001", "items": [{"product_codigo": "P00001", "quantity": 2}]})

    UpdateOrderStatusUseCase(repository=order_repo, inventory_repo=repo, order_item_repo=item_repo).execute(
        order.codigo, "CANCELLED"
    )
    ...
    returns = (
        db.query(StockMovementModel)
        .filter(StockMovementModel.reference_id == order.codigo, StockMovementModel.type == "RETURN")
        .count()
    )
    assert sales == 0 and returns == 0


def test_cancel_confirmed_no_stock_change(db):
    """CONFIRMED → CANCELLED: no-op (Decisão B3a — nada foi debitado)."""
    seed_product(db)
    seed_client(db)
    repo = SQLAlchemyInventoryRepository(db)
    repo.add_stock_atomic("P00001", 10, reason="Initial")
...
def test_order_idempotent_deduction(db):
    """Same order confirmed twice → 0 SALE, stock = 10 (Decisão B3a:
    pedido não debita; idempotência de débito é da entrega)."""
    seed_product(db)
    seed_client(db)
    repo = SQLAlchemyInventoryRepository(db)
    repo.add_stock_atomic("P00001", 10, reason="Initial")
    ...
    order = CreateOrderUseCase(
        ...
        product_repo=SQLAlchemyProductRepository(db),
        inventory_repo=repo,
    ).execute({"client_codigo": "000001", "items": [{"product_codigo": "P00001", "quantity": 3}]})
    ...
    sales = (
        db.query(StockMovementModel)
        .filter(StockMovementModel.type == "SALE", StockMovementModel.reference_id == order.codigo)
        .count()
    )
    assert sales == 0
...
def test_adv07_pending_cancel_no_return(db):
    ...
    or_ = SQLAlchemyOrderRepository(db)
    ir = SQLAlchemyOrderItemRepository(db)
    ...
    assert (
        db.query(StockMovementModel)
        .filter(StockMovementModel.reference_id == o.codigo, StockMovementModel.type == "RETURN")
        .count()
        == 0
    )
...
def test_adv08_no_partial_multi_item(db):
    ...
    seed_product(db)
    db.add(ProductModel(codigo="P00002", nome="B", tipo="GAS", preco=50, estoque=0))
    db.commit()
    seed_client(db)
    repo = SQLAlchemyInventoryRepository(db)
    repo.add_stock_atomic("P00001", 10, reason="A")
    ...
    with pytest.raises(ValueError):
        CreateOrderUseCase(
            ...
            {
                "client_codigo": "000001",
                "items": [{"product_codigo": "P00001", "quantity": 2}, {"product_codigo": "P00002", "quantity": 2}],
            }
        )
    assert db.query(InventoryModel).filter(InventoryModel.product_codigo == "P00001").first().quantity == 10
...
def test_adv11_ledger_reconstructs(db):
    ...
    movements = db.query(StockMovementModel).filter(StockMovementModel.product_codigo == "P00001").all()
    balance = sum(
        m.quantity if m.type in ("ENTRY", "RETURN") else -m.quantity for m in movements if m.type != "ADJUSTMENT"
    )

...
def _login_driver(store, driver_id="drv-a"):
    ...
    store["sessions"][token] = {
        ...
        "expires_at": (datetime.utcnow().replace(
            hour=min(23, datetime.utcnow().hour + 8)
        )).isoformat(),
    }
    return token
...
class TestDriverDeliveries:
    def test_list_own_deliveries(self, store, setup_data):
        ...
        own = [d for d in store["deliveries"].values()
               if d.get("driver_id") == "drv-a" and d.get("tenant_id") == "default"]
        assert len(own) == 0  # del-a has no driver yet

    def test_driver_b_sees_own(self, store, setup_data):
        token = _login_driver(store, "drv-b")
        own = [d for d in store["deliveries"].values()
               if d.get("driver_id") == "drv-b" and d.get("tenant_id") == "default"]
        assert len(own) == 1
        assert own[0]["id"] == "del-b"

    def test_no_cross_driver(self, store, setup_data):
        """Driver A cannot see Driver B's deliveries."""
        own_a = [d for d in store["deliveries"].values()
                 if d.get("driver_id") == "drv-a"]
        own_b = [d for d in store["deliveries"].values()
                 if d.get("driver_id") == "drv-b"]
    ...
    def test_no_cross_tenant(self, store, setup_data):
        ...
        own_default = [d for d in store["deliveries"].values()
                       if d.get("tenant_id") == "default"]
        other = [d for d in store["deliveries"].values()
                 if d.get("tenant_id") == "other-tenant"]
...
class TestAdversarial:
    ...
    def test_38_no_secrets(self):
        ...
        dto = DriverDeliverySummary(
            delivery_id="d1", order_reference="O1", customer_name="M",
            address="Rua A", status="PENDING",
        )
        data = dto.model_dump()

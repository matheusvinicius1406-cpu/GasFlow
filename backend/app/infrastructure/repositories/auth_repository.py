...
class SQLAlchemyUserRepository:
    ...
    def list_by_tenant(self, tenant_id: str) -> List[AuthUserModel]:
        ...
        membership_user_ids = select(AuthMembershipModel.user_id).where(AuthMembershipModel.tenant_id == tenant_id)
        return self.db.query(AuthUserModel).filter(AuthUserModel.id.in_(membership_user_ids)).all()
...
class SQLAlchemySessionRepository:
    ...
    def get_by_id(self, session_id: str) -> Optional[AuthSessionModel]:
        return self.db.query(AuthSessionModel).filter(AuthSessionModel.id == session_id).first()
    ...
    def revoke_by_token(self, token: str) -> bool:
        model = self.db.query(AuthSessionModel).filter(AuthSessionModel.token == token).first()
        if not model:
            return False
        model.status = "REVOKED"
        model.revoked_at = datetime.utcnow()
...
class SQLAlchemyTenantRepository:
    ...
    def get_by_tenant_id(self, tenant_id: str) -> Optional[AuthTenantModel]:
        return self.db.query(AuthTenantModel).filter(AuthTenantModel.tenant_id == tenant_id).first()
...
class SQLAlchemyRoleRepository:
    ...
    def create(self, role_id: str, name: str, system_role: str, permissions: List[str] = None) -> AuthRoleModel:
        model = AuthRoleModel(
            id=role_id,
            name=name,
...
class SQLAlchemyMembershipRepository:
    ...
    def get(self, user_id: str, tenant_id: str) -> Optional[AuthMembershipModel]:
        return (
            self.db.query(AuthMembershipModel)
            .filter(
    ...
    def list_for_tenant(self, tenant_id: str) -> List[AuthMembershipModel]:
        return self.db.query(AuthMembershipModel).filter(AuthMembershipModel.tenant_id == tenant_id).all()
...
class SQLAlchemyAuditRepository:
    ...
    def list_for_tenant(self, tenant_id: str, limit: int = 50) -> List[AuthAuditModel]:
        return (
            self.db.query(AuthAuditModel)
            .filter(AuthAuditModel.tenant_id == tenant_id)

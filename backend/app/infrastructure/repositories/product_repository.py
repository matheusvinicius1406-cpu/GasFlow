...
class SQLAlchemyProductRepository(ProductRepository):
    ...
    def buscar_por_codigo(self, codigo: str) -> Optional[Product]:
        model = self.db.query(ProductModel).filter(ProductModel.codigo == codigo).first()
    ...
    def listar_todos(self) -> List[Product]:
        models = self.db.query(ProductModel).filter(ProductModel.ativo == True).all()
    ...
    def proximo_codigo(self) -> str:
        last = self.db.query(ProductModel).order_by(ProductModel.id.desc()).first()
        if not last:
            return "000001"

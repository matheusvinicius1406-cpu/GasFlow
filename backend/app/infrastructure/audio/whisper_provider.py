...
class WhisperSTTProvider(SpeechToTextProvider):
    ...
    def transcribe(self, audio_bytes: bytes, mime_type: str = "audio/ogg",
        ...
        if not audio_bytes:
            return TranscriptionResult(text="", error="AUDIO_EMPTY", language=language,
                                       provider=self.provider_name)
        if not self.health_check():
            return TranscriptionResult(text="", error="STT_UNAVAILABLE",
                                       language=language, provider=self.provider_name)
        ...
        with tempfile.TemporaryDirectory(prefix="gasflow_whisper_") as tmp_dir:
            ...
            cmd = [
                self._executable, str(audio_path),
                "--model", self._model,
                "--language", lang,
                "--output_format", "txt",
                "--output_dir", tmp_dir,
            ...
            try:
                ...
                return TranscriptionResult(text="", error=f"STT_ERROR:{type(exc).__name__}",
                                           language=language, provider=self.provider_name)
            if result.returncode != 0:
                return TranscriptionResult(text="", error="STT_ERROR:nonzero_exit",
                                           language=language, provider=self.provider_name)
            ...
            return TranscriptionResult(
                text=text, confidence=1.0, language=language,
                duration_seconds=0.0, provider=self.provider_name,
            )
